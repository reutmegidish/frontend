...
const CarAgencyManager = {
  ...
  searchAgency(idOrName) {
    const agency = agencies.find(
      (agency) => agency.agencyId === idOrName || agency.agencyName === idOrName
    );
    return agency || null;
  },
  ...
  getAllAgencies() {
    const agenciesArr = agencies.reduce((arr, agency) => {
      arr.push(agency.agencyName);
      return arr;
    }, []);
    return agenciesArr;
  },
  ...
  updateCarPrice(agencyId, carId, newPrice) {
    ...
    if (agency) {
      const carToUpdate = agency.cars.find((car) =>
        car.models.some((model) => model.carNumber === carId)
      );
      if (carToUpdate) {
        const modelToUpdate = carToUpdate.models.find(
  ...
  getTotalAgencyRevenue(agencyId) {
    ...
    if (agency) {
      return agency.cars.reduce((total, car) => {
        return (
          total +
          car.models.reduce((subTotal, model) => model.price + subTotal, 0)
        );
      }, 0);
    }
    return 0;
